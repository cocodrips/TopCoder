...
def do_test(number, __expected):
    startTime = time.time()
    instance = SplitStoneGame()
    exception = None
    try:
        __result = instance.winOrLose(number);
    except:
        import traceback
...
def run_tests():
    sys.stdout.write("SplitStoneGame (250 Points)\n\n")
    ...
    with open("SplitStoneGame.sample", "r") as f:
        while True:
            label = f.readline()
