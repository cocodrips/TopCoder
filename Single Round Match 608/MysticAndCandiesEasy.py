...
def do_test(C, X, high, __expected):
    startTime = time.time()
...
def run_tests():
    ...
    with open("MysticAndCandiesEasy.sample", "r") as f:
        while True:
            label = f.readline()
            ...
            for i in range(0, int(f.readline())):
                high.append(int(f.readline().rstrip()))
            high = tuple(high)
            f.readline()
            ...
            passed += do_test(C, X, high, __answer)

    sys.stdout.write("\nPassed : %d / %d cases\n" % (passed, cases))

    T = time.time() - 1391789281
    PT, TT = (T / 60.0, 75.0)
